...
**YOU MUST COMPLETE ALL STEPS BELOW BEFORE CALLING mcp__atlassian__jira_create_issue**
...
// CRITICAL field values for WAO project:
...
mcp__atlassian__jira_create_issue_link({
  type: "Blocks",
  inward_issue: "WAO-AAA",
  outward_issue: "WAO-BBB"
})
...
Before calling `mcp__atlassian__jira_create_issue`, verify:
...
If context window is running low during planning, follow the Checkpoint Protocol (`docs/checkpoint.md`):
save current planning state, then end the session.
